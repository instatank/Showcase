  ...
  return (
    <>
      ...
      >
        {children}
        {/* hover hint */}
        <span className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-ink/80 px-3 py-1 text-xs font-medium text-white opacity-0 backdrop-blur transition-opacity duration-300 group-hover/zoom:opacity-100">
          🔍 Click to expand
        </span>
      </div>
      ...
      {open && (
        <div
          ...
          className="fixed inset-0 z-[60] flex flex-col items-center justify-center gap-4 bg-ink/90 p-4 backdrop-blur-md sm:p-8"
        >
          <button
            type="button"
            aria-label="Close"
